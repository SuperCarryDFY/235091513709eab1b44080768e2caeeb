...
def extract_texts(data_dict: dict):
    ...
    for comment in data_dict.get("comments", []):
        ...
        if comment["commentType"] == "CATALYTIC ACTIVITY":
            ...
            note = f"{sep}".join(
                ["molecule", molecule, "physiologicalReactions", physiologicalReactions]
            )
    ...
    for feature in data_dict.get("features", []):
        ...
        if feature["type"] == "Binding site":
            ...
            ligand_part = (
                feature["ligandPart"]["name"] if "ligandPart" in feature else None
            )
            evidences = json.dumps(feature.get("evidences", None))
        ...
        if feature["type"] == "Mutagenesis":
            ...
            note = f"{sep}".join(
                ["start", str(st), "end", str(ed), "ori", ori_seq, "mut", alt_seq]
            )
            evidences = json.dumps(feature.get("evidences", None))
        ...
        if feature["type"] == "Disulfide bond":
            ...
            note = f"{sep}".join(
                ["start", str(st), "end", str(ed), "start_aa", st_aa, "end_aa", ed_aa]
            )
            evidences = json.dumps(feature.get("evidences", None))
...
def record2text(record: list, template: dict, template_id: int = None) -> str or list:
    ...
    if subsection == "Catalytic activity":
        text = fill_template(template, template_id)
    ...
    if subsection == "Gene names":
        if note == "orfNames" or note == "orderedLocusNames":
            text = fill_template(template, template_id, note)
        else:
            text = fill_template(template, template_id, "normal")
    ...
    if subsection == "Natural variant":
        ...
        if raw_text == "":
            text = fill_template(template, template_id, extra_key="without raw_text")
        else:
            text = fill_template(template, template_id, extra_key="with raw_text")
    ...
    if subsection == "Cross-link":
        ...
        if st == ed:
            text = fill_template(template, template_id, "interchain")
        else:
            text = fill_template(template, template_id, "intrachain")
